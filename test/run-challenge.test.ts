...
describe("Pi launch", () => {
  ...
  it("appends structurally consistent public journey guidance to Pi's built-in system prompt", async () => {
    ...
    const skill = await readFile(path.resolve("solution/skills/mvp-builder/SKILL.md"), "utf8");
    const args = buildPiArguments(
      "Build a tool",
      systemPrompt,
      publicJourneys,
    ...
    // The composed prompt must keep both guardrails: no invented features, no dropped journeys.
    expect(suppliedSystemPrompt).toContain("skip a pattern it does not imply rather than inventing a substitute");
    expect(suppliedSystemPrompt).toContain("never drop a journey it does imply");
    // The skill is inlined rather than passed as a --skill path, so it costs no read turn.
    expect(args).not.toContain("--skill");
    expect(suppliedSystemPrompt).toContain("Shaping any idea into a POC");
