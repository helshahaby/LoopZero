...
/**
 * Sums usage across every Pi invocation of a run. The harness may call Pi more than once
 * (initial build plus bounded repair), and the judge recomputes totals from all event
 * streams, so aggregation must be exact and the call log must stay ordered and re-indexed.
 */
export function mergeUsageSummaries(...summaries: UsageSummary[]): UsageSummary {
  const merged: UsageSummary = { ...ZERO_USAGE, call_log: [] };
  ...
  merged.cost_total = Number(merged.cost_total.toFixed(6));
  return merged;
}
